...
export class FilterCondition {
    ...
    @ApiPropertyOptional({
        description: 'Filter value (single value, array for IN, or [from, to] for BETWEEN)',
...
function parseShorthandCondition(chunk: string): FilterCondition | null {
    ...
    if (parts.length === 1) {
        return new FilterCondition({ field: parts[0], operator: FilterOperator.EQ, value: true });
    }
    if (parts.length === 2) {
        return new FilterCondition({ field: parts[0], operator: FilterOperator.EQ, value: parts[1] });
    }
    const operator = (parts[1].toUpperCase() as FilterOperator) in FilterOperator
...
export class FilteringQueryDto {
    @ApiPropertyOptional({
        description: 'Filter conditions (as JSON string or shorthand: field:operator:value)',
