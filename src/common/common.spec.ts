...
import { RequestContextService, RequestContextStore } from './services/request-context.service';
import { PaginationQueryDto, PaginatedResult, sanitizeSortColumn } from './dto/pagination.dto';
...
describe('Step 0 Cross-Cutting Foundation', () => {
    ...
    describe('RequestContextService', () => {
        it('should propagate correlationId, user, and IP within context', (done) => {
            ...
            RequestContextService.run(store, () => {
                ...
                expect(requestContextService.getUserAgent()).toBe('Mozilla/5.0 JestTest');
                expect(requestContextService.getDurationMs()).toBeGreaterThanOrEqual(50);
                done();
            });
        });
    });
    ...
    describe('TransformInterceptor', () => {
        it('should envelope standard objects with success: true and metadata', (done) => {
            const interceptor = new TransformInterceptor(reflector, requestContextService);
            const context: any = {
                getHandler: () => ({}),
                getClass: () => ({}),
            };
    ...
    describe('Pagination & Sorting Helpers', () => {
        ...
        it('should sanitize sort column safely', () => {
            ...
            expect(sanitizeSortColumn('DROP TABLE Users;--', allowed, 'CreatedAt')).toBe('CreatedAt');
    ...
    describe('Filtering Framework', () => {
        it('should parse shorthand filter strings correctly', () => {
            const shorthand = 'EventType:EQ:LOGIN_FAILURE,CreatedAt:GTE:2026-08-01,Status:IN:ACTIVE|PENDING';
            const parsed = parseFilterQuery(shorthand);
            expect(parsed).toHaveLength(3);
            expect(parsed[0]).toEqual({ field: 'EventType', operator: FilterOperator.EQ, value: 'LOGIN_FAILURE' });
            expect(parsed[1]).toEqual({ field: 'CreatedAt', operator: FilterOperator.GTE, value: '2026-08-01' });
            expect(parsed[2]).toEqual({ field: 'Status', operator: FilterOperator.IN, value: ['ACTIVE', 'PENDING'] });
        });
        ...
        it('should parse JSON filter strings correctly', () => {
            ...
            expect(parsed[0]).toEqual({ field: 'Role', operator: 'EQ', value: 'SYSTEM_ADMIN' });
            expect(parsed[1]).toEqual({ field: 'Attempts', operator: 'GT', value: 3 });
        });

        it('should build parameterized SQL Server WHERE clause safely with column whitelisting', () => {
            const filters = [
                { field: 'eventType', operator: FilterOperator.EQ, value: 'LOGIN_SUCCESS' },
                { field: 'userId', operator: FilterOperator.EQ, value: 'usr-123' },
                { field: 'createdAt', operator: FilterOperator.GTE, value: '2026-08-01T00:00:00Z' },
                { field: 'status', operator: FilterOperator.IN, value: ['ACTIVE', 'LOCKED'] },
                { field: 'maliciousColumn', operator: FilterOperator.EQ, value: 'hack' }, // not in whitelist
            ];
        ...
        it('should format HttpException as standard ErrorEnvelope', () => {
            ...
            const host: any = {
                switchToHttp: () => ({
                    ...
                    getRequest: () => ({ originalUrl: '/api/v1/bad-request', url: '/api/v1/bad-request' }),
                }),
            };
