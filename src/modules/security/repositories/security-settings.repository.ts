...
export class SecuritySettingsRepository {
    ...
    async updateSetting(settingCode: string, settingValue: string, updatedBy?: string | null): Promise<void> {
        const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        const validUpdatedBy = updatedBy && UUID_REGEX.test(updatedBy) ? updatedBy : null;
        ...
        const query = `
            ...
            WHERE SettingCode = @0
        `;
        await this.dataSource.query(query, [settingCode, settingValue, validUpdatedBy]);
    }
