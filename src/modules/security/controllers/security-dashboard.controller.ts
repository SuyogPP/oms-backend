...
export class SecurityDashboardController {
    constructor(private readonly securityDashboardService: SecurityDashboardService) {}
    ...
    @Get('user-summary')
    @ApiOperation({ summary: "Get current authenticated user's 30-day security summary" })
    async getUserSummary(@CurrentUser() user: ICurrentUser): Promise<SecuritySummaryDto> {
        if (!user?.userId) {
    ...
    @ApiOperation({ summary: 'Get paginated, filtered, and sorted security events' })
    async getEvents(@Query() query: BaseQueryDto): Promise<PaginatedResult<SecurityEventDto>> {
        return this.securityDashboardService.getSecurityEvents(query);
    }

    @Get('failed-logins')
    @RequirePermissions(PERMISSIONS.SECURITY_FAILED_LOGINS_VIEW)
    @ApiOperation({ summary: 'Get paginated, filtered, and sorted failed login attempts' })
    async getFailedLogins(@Query() query: BaseQueryDto): Promise<PaginatedResult<FailedLoginAttemptDto>> {
    ...
    @BypassTransform()
    @RequirePermissions(PERMISSIONS.SECURITY_DASHBOARD_VIEW)
    @ApiOperation({ summary: 'Server-Sent Events (SSE) stream for real-time security dashboard updates' })
