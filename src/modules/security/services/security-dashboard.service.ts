...
export class SecurityDashboardService {
  ...
  async getDashboardData(): Promise<SecurityDashboardDataDto> {
    const defaultQuery = Object.assign(new BaseQueryDto(), {
      page: 1,
      pageSize: 25,
    });
    const [summary, eventsResult, failedLoginsResult, activeSessions] =
      await Promise.all([
  ...
  getEventStream(): Observable<MessageEvent> {
    ...
    const initial$ = from(this.getFullCompositeStreamData()).pipe(
      ...
      catchError((err) => {
        ...
        return of({
          data: { error: 'Failed to fetch initial security stream data' },
        } as MessageEvent);
      }),
    );
    ...
    const periodic$ = interval(30000).pipe(
      ...
      catchError((err) => {
        ...
        return of({
          data: { error: 'Failed to fetch periodic security stream data' },
        } as MessageEvent);
      }),
    );
