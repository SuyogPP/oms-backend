...
export class UserSessionsService {
    ...
    async getUserSessions(userId: string, currentSessionId?: string | null): Promise<UserSessionsListResponseDto> {
        const rows = await this.repository.getActiveSessionsByUserId(userId);

        const currentSidUpper = currentSessionId ? currentSessionId.toUpperCase() : null;

        const sessions: UserActiveSessionDto[] = rows.map((row) => {
            const sidUpper = row.LoginSessionID.toUpperCase();
            const isCurrent = Boolean(currentSidUpper && sidUpper === currentSidUpper);
    ...
    async revokeSession(
        ...
        if (currentSessionId && sessionId.toUpperCase() === currentSessionId.toUpperCase()) {
    ...
    async revokeAllOtherSessions(
        ...
        const revokedSessions = await this.repository.revokeAllOtherSessions(userId, currentSessionId);
