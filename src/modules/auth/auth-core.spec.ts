...
describe('AuthCoreModule (Step 4)', () => {
    ...
    beforeEach(async () => {
        mockRepository = {
            ...
            findSessionByRefreshTokenHash: jest.fn().mockImplementation(async (hash: string) => {
                const validHash = crypto.createHash('sha256').update('valid-token').digest('hex');
                const replayedHash = crypto.createHash('sha256').update('replayed-token').digest('hex');
        ...
        const module: TestingModule = await Test.createTestingModule({
            ...
            providers: [
                ...
                { provide: SecuritySettingsService, useValue: mockSecuritySettingsService },
    ...
    describe('login', () => {
        it('should successfully authenticate valid credentials and issue tokens', async () => {
            ...
            expect(mockRepository.resetFailedLogin).toHaveBeenCalledWith('1053433E-F36B-1410-85ED-009A959FB122');
            expect(mockSecurityEventsService.log).toHaveBeenCalledWith(
                'LOGIN_SUCCESS',
                expect.objectContaining({ userId: '1053433E-F36B-1410-85ED-009A959FB122' }),
            );
        });

        it('should reject unknown username and log failed attempt', async () => {
            await expect(
                service.login({ username: 'nonexistent', password: 'Password123!' }, '127.0.0.1', 'Jest'),
            ).rejects.toThrow(UnauthorizedException);

            expect(mockRepository.createFailedLoginAttempt).toHaveBeenCalledWith(
                expect.objectContaining({ username: 'nonexistent', failureReason: 'INVALID_USERNAME' }),
            );
        });

        it('should reject locked account', async () => {
            await expect(
                service.login({ username: 'locked_user', password: 'Password123!' }, '127.0.0.1', 'Jest'),
            ).rejects.toThrow(UnauthorizedException);

            expect(mockRepository.createFailedLoginAttempt).toHaveBeenCalledWith(
                expect.objectContaining({ username: 'locked_user', failureReason: 'ACCOUNT_LOCKED' }),
            );
        });

        it('should reject invalid password and record failure', async () => {
            await expect(
                service.login({ username: 'admin', password: 'WrongPassword!' }, '127.0.0.1', 'Jest'),
            ).rejects.toThrow(UnauthorizedException);

            expect(mockRepository.recordFailedLogin).toHaveBeenCalledWith('1053433E-F36B-1410-85ED-009A959FB122');
            expect(mockRepository.createFailedLoginAttempt).toHaveBeenCalledWith(
                expect.objectContaining({ username: 'admin', failureReason: 'INVALID_PASSWORD' }),
            );
        });
        ...
        it('should handle concurrent session limit by requesting confirmation when autoRevoke is enabled', async () => {
            ...
            await expect(
                service.login(
                    { username: 'admin', password: 'Password123!', confirmRevokeOldest: false },
                    '127.0.0.1',
                    'Jest',
                ),
        ...
        it('should auto-revoke oldest session when confirmRevokeOldest is true', async () => {
            ...
            const result = await service.login(
                { username: 'admin', password: 'Password123!', confirmRevokeOldest: true },
                '127.0.0.1',
                'Jest',
            );

            expect(result.success).toBe(true);
            expect(mockRepository.revokeSession).toHaveBeenCalledWith('oldest-session-id');
            expect(mockSecurityEventsService.log).toHaveBeenCalledWith(
    ...
    describe('refresh', () => {
        it('should rotate refresh token and issue new access token', async () => {
            const result = await service.refresh({ refreshToken: 'valid-token' }, '127.0.0.1', 'Jest');

            expect(result.success).toBe(true);
            expect(result.accessToken).toBe('mock.jwt.token');
            expect(result.refreshToken).toBeDefined();
            expect(mockRepository.revokeRefreshToken).toHaveBeenCalledWith('sess-active');
        ...
        it('should detect replay attack on revoked refresh token and execute security actions', async () => {
            await expect(
                service.refresh({ refreshToken: 'replayed-token' }, '127.0.0.1', 'Jest'),
            ).rejects.toThrow(ForbiddenException);

            expect(mockRepository.revokeSession).toHaveBeenCalledWith('sess-replayed');
            expect(mockSecurityEventsService.log).toHaveBeenCalledWith(
                'REFRESH_TOKEN_REPLAY',
                expect.objectContaining({ loginSessionId: 'sess-replayed' }),
            );
            expect(mockRepository.revokeAllSessionsForUser).toHaveBeenCalledWith('1053433E-F36B-1410-85ED-009A959FB122');
        });
    });

    describe('logout', () => {
        it('should revoke session, record logout history, and log security event', async () => {
            const result = await service.logout('sess-active', '1053433E-F36B-1410-85ED-009A959FB122', '127.0.0.1', 'Jest');
            ...
            expect(mockRepository.createLogoutHistory).toHaveBeenCalledWith(
                expect.objectContaining({ loginSessionId: 'sess-active', logoutReason: 'USER_LOGOUT' }),
            );
            expect(mockSecurityEventsService.log).toHaveBeenCalledWith(
                'LOGOUT',
                expect.objectContaining({ loginSessionId: 'sess-active' }),
