...
export class RetentionService {
    ...
    async handleCron(): Promise<void> {
        this.logger.log('Executing automated daily data retention cleanup cron job...');
        try {
            const result = await this.executeCleanup('CRON_SCHEDULER');
            this.logger.log(`Retention cleanup completed: ${result.message}`);
        } catch (error) {
            this.logger.error(`Automated retention cleanup failed: ${(error as Error).message}`, (error as Error).stack);
        }
    }

    async executeCleanup(triggeredBy: string = 'MANUAL'): Promise<RetentionCleanupResultDto> {
        this.logger.log(`[RETENTION] Starting retention cleanup job (triggered by: ${triggeredBy})...`);
        ...
        const securityEventsDeleted = await this.repository.purgeSecurityEvents(securityEventsDays);

        // 2. Purge Login History
        const loginHistoryDeleted = await this.repository.purgeLoginHistory(loginHistoryDays);

        // 3. Purge Logout History
        const logoutHistoryDeleted = await this.repository.purgeLogoutHistory(logoutHistoryDays);

        // 4. Purge Failed Login Attempts
        const failedLoginsDeleted = await this.repository.purgeFailedLogins(failedLoginsDays);

        // 5. Purge Inactive Sessions
        const loginSessionsDeleted = await this.repository.purgeInactiveSessions(securityEventsDays);
        ...
        await this.securityEventsService.log(SECURITY_EVENTS.RETENTION_JOB_EXECUTED, {
