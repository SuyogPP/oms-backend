...
describe('UserCredentialsController (Domain 3, §§5.2, 5.3, 5.4, 8)', () => {
  ...
  it('POST /authorization/users/:id/invite -> delegates to service.inviteUser', async () => {
    mockService.inviteUser.mockResolvedValueOnce({
      success: true,
  ...
  it('POST /authorization/users/:id/unlock -> delegates to service.unlockUser', async () => {
    mockService.unlockUser.mockResolvedValueOnce({
      success: true,
      message: 'Unlocked',
    });
  ...
  it('POST /authorization/invitations/:token/validate -> delegates to service.validateInvitationToken', async () => {
    mockService.validateInvitationToken.mockResolvedValueOnce({
      valid: true,
      purpose: 'INVITE',
    });
  ...
  it('POST /authorization/invitations/:token/accept -> delegates to service.acceptInvitation', async () => {
    mockService.acceptInvitation.mockResolvedValueOnce({
      success: true,
      message: 'Accepted',
    });

    const result = await controller.acceptInvitation('valid-token', {
      password: 'StrongP@ssw0rd!',
    });
    expect(service.acceptInvitation).toHaveBeenCalledWith('valid-token', {
      password: 'StrongP@ssw0rd!',
    });
    expect(result.success).toBe(true);
  });
